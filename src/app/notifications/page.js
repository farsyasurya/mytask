...
export default function NotificationPage() {
    ...
    const handleNotificationClick = async (notif) => {
        ...
        // Redirect ke halaman task (bisa bawa query taskId jika ada)
        if (notif.taskId) {
    ...
    return (
        <div className="flex min-h-screen bg-slate-50 dark:bg-slate-950 text-slate-900 dark:text-slate-100 pb-20 md:pb-0">
            <Sidebar />

            <main className="flex-1 p-4 sm:p-6 md:p-8 max-w-4xl w-full space-y-6">
                <div>
                    ...
                    {notifications.length === 0 ? (
                        ...
                        notifications.map((notif) => (
                            ...
                            <div
                                ...
                                className={`p-4 rounded-xl border transition-all cursor-pointer flex items-center justify-between gap-4 ${notif.read
                            ...
                            >
                                <div className="flex items-start gap-3.5 min-w-0">
                                    <div
                                        className={`w-2.5 h-2.5 rounded-full mt-1.5 flex-shrink-0 ${notif.read ? "bg-slate-300 dark:bg-slate-700" : "bg-indigo-600 animate-pulse"
                                            }`}
                                    />
